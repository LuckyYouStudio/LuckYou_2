...
- **FR-C-28** 滚存缓冲区（2026-08-10 第二/四轮自查增）：`s_pendingPot` 与
  ...
  - ⚠️ **缓冲区没有退出通道，这是刻意的（2026-08-14 决定）**。
    出口唯一：`_openNextRound`。若彩票因任何原因永久停摆，缓冲区内尚未分配的资金
    **将永久留在合约中，任何人都无法取出**——包括运营方。
    - **为什么不加**：这笔钱没有干净的归属。四个来源里只有**注资**有明确原主
      （`PotInjected` 记了 sender）；过期未领奖金的原主是**主动放弃了**的中奖者
      （90 天没领），未开出奖级与取整余数来自那期购票者、而他们已经参与过开奖。
      更关键的是，**任何「分给某些人」的规则都会制造捕获动机**：例如「弃期时按票数
      分给该期购票者」会被这样打穿——往一个注定被弃期的期里砸一大笔票，30 天后
      票款照常全额退回，同时按比例吃掉整个缓冲区，成本只有 gas 和资金占用。
      这与本项目栽过五次的 carry 捕获是同一类问题。
    - **缓解手段是运营纪律而非代码**：见 FR-C-26——单次注资不应超过一期的有机售票额。
      健康运转时缓冲区只是余数、未开出奖级与被扣留的超额，相对流水很小；
      真正会让它变大的场景只有「大额冷启动注资长期没被分掉」。
    - **MUST 向用户提前告知**：前端 **MUST** 有可见的风险提示说明这一点，
      而不是只写在本文档里。把它当作已披露的不可抗力，而不是一个待修的缺陷
...
- **Q9（VRF 换源杠杆的根因修复，2026-08-11 提出；2026-08-13 已决：采纳 B + C）**：
  ...
  **补充已决（2026-08-14）：C 的「配套缓冲区逃生规则」决定不做**，
  改为按不可抗力如实提前告知（见 FR-C-28 的说明与其中的捕获动机分析）。
